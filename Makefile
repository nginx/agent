...
generate-pgo-profile: build-mock-management-plane-grpc load-test-image
...
#	$(GOTEST) -v ./test/integration/managementplane -cpuprofile integration_cpu.pprof
#
...
run-load-test-image: ## Run performance load testing image
	...
	@$(CONTAINER_BUILDENV) $(CONTAINER_CLITOOL) run \
		...
		-v $(PWD)/$(BUILD_DIR)/:/agent/$(BUILD_DIR)/ \
...
run-load-test-with-cpu-profiling: ## Run performance load testing with cpu profiling
	...
	@echo "Benchmarks and profile saved to $(TEST_BUILD_DIR)/load-cpu-profiling"
	@echo "You can use 'go tool pprof <binary> <profile>' to analyze the profile"
