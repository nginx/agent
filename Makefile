...
new-integration-test:
	PACKAGE_NAME=${PACKAGE_NAME} CONTAINER_NGINX_IMAGE_REGISTRY=${CONTAINER_NGINX_IMAGE_REGISTRY} ARCH=${OSARCH} OS_VERSION=${OS_VERSION} \
		...
		${GOTEST} -v ./test/integration/api
	PACKAGE_NAME=${PACKAGE_NAME} CONTAINER_NGINX_IMAGE_REGISTRY=${CONTAINER_NGINX_IMAGE_REGISTRY} ARCH=${OSARCH} OS_VERSION=${OS_VERSION} \
    	OS_RELEASE=${OS_RELEASE} TAG=${NGINX_TAG} CONTAINER_OS_TYPE=${CONTAINER_OS_TYPE} IMAGE_PATH=${AGENTLESS_IMAGE_PATH} DOCKER_COMPOSE_FILE="docker-compose-${CONTAINER_OS_TYPE}-new.yml" \
    	${GOTEST} -v ./test/integration/install
	PACKAGE_NAME=${PACKAGE_NAME} CONTAINER_NGINX_IMAGE_REGISTRY=${CONTAINER_NGINX_IMAGE_REGISTRY} ARCH=${OSARCH} OS_VERSION=${OS_VERSION} \
	    OS_RELEASE=${OS_RELEASE} TAG=${NGINX_TAG} CONTAINER_OS_TYPE=${CONTAINER_OS_TYPE} IMAGE_PATH=${AGENT_IMAGE_PATH} DOCKER_COMPOSE_FILE="docker-compose-${CONTAINER_OS_TYPE}-new.yml" \
		${GOTEST} -v ./test/integration/features
