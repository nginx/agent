...
func (m *MockNginxBinary) ValidateConfig(processId, bin, configLocation string, errorLogPaths []string, config *proto.NginxConfig, configApply *sdk.ConfigApply) error {
	args := m.Called(processId, bin, configLocation, errorLogPaths, config, configApply)
	return args.Error(0)
}
