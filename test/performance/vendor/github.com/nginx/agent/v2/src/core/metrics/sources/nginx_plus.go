...
func (c *NginxPlus) Collect(ctx context.Context, wg *sync.WaitGroup, m chan<- *proto.StatsEntity) {
	...
	c.init.Do(func() {
		...
		if err != nil {
			logMetricCollectionError(fmt.Sprintf("Failed to create plus metrics client, %v", err))
			SendNginxDownStatus(ctx, c.baseDimensions.ToDimensions(), m)
			return
		}
		c.prevStats, err = cl.GetStats()
		if err != nil {
			logMetricCollectionError(fmt.Sprintf("Failed to retrieve plus metrics, %v", err))
			SendNginxDownStatus(ctx, c.baseDimensions.ToDimensions(), m)
			c.prevStats = nil
			return
		}
