...
- To enable mTLS, you must have TLS enabled and supply a key, cert, and a CA cert on both the client and server. See the [Secure Client Acces and Network Traffic](https://docs.nginx.com/nginx-management-suite/admin-guides/getting-started/secure-traffic/) topic for instructions on how to generate keys and set them in the specific values in the NGINX Agent configuration.
