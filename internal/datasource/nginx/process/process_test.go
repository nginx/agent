...
func TestGetInfo(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			nginxVersionCommandOutput: `nginx version: nginx/1.23.3
		...
		},
		{
			...
			expected: &Info{
				...
				ConfigureArgs: []string{
					...
					"with-cc-opt='-g -O2 -fdebug-prefix-map=/data/builder/debuild/nginx-plus-1.25.1/debian/debuild-base/nginx-plus-1.25.1=. -fstack-protector-strong -Wformat -Werror=format-security -Wp,-D_FORTIFY_SOURCE=2 -fPIC'",
					"with-ld-opt='-Wl,-Bsymbolic-functions -Wl,-z,relro -Wl,-z,now -Wl,--as-needed -pie'",
				},
				ExePath: "/usr/sbin/nginx",
			},
	...
	for _, test := range tests {
		t.Run(test.name, func(tt *testing.T) {
			...
			n := New(mockExec)
			result, err := n.GetInfo(123, test.exePath)
