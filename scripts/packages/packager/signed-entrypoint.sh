...
for freebsd_abi in $FREEBSD_DISTROS; do \
    ...
    cd ../../../../; \
    cp ./build/packages/txz/"$freebsd_abi"/nginx-agent-"${VERSION}".pkg ./build/github/packages/nginx-agent-"${VERSION}"-"$freebsd_abi".pkg; \
    cp ./build/packages/txz/"$freebsd_abi"/nginx-agent-"${VERSION}".pkg ./build/azure/packages/nginx-agent-"${VERSION}"-"${freebsd_abi//:}".pkg; \
done; \
